...
function App() {
  ...
  return (
    <div className='flex flex-col p-2 gap-2 min-h-screen'>
      <div className='mx-auto border border-black py-2 px-20 min-w-[25%] text-center'>Экран</div>

      <div className='flex-1 flex justify-between items-start'>
        <div className='flex flex-wrap'>
          {tables.map((_, index) => (
            <div className='p-1 w-1/2' key={index + 7}>
              <Table id={index + 7} />
            </div>
          ))}
        </div>
        ...
        <div className='flex flex-wrap'>
          {tables.map((_, index) => (
            <div className='p-1 w-1/2' key={index + 1}>
              <Table id={index + 1} />
            </div>
          ))}
        </div>
      ...
      <div className='flex justify-center gap-x-36 items-end'>
